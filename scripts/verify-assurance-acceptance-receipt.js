...
export async function verifyAssuranceAcceptanceReceiptFile({ receiptPath, signingPublicKeyPath } = {}) {
  if (!receiptPath || !signingPublicKeyPath) throw new TypeError('receiptPath and signingPublicKeyPath are required.');
...
export function verifyAssuranceAcceptanceReceipt(receipt, signingPublicKeyPem) {
...
async function main(argv) {
  const [receiptPath, signingPublicKeyPath] = argv;
  const result = await verifyAssuranceAcceptanceReceiptFile({ receiptPath, signingPublicKeyPath });
