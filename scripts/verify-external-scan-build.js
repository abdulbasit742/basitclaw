...
await requireMarkers('src/evidence/externalScanCallbackHandler.js', 'External scan callback', [
  ...
  'HMAC realm', 'external_scan.authentication_failed', 'EXTERNAL_SCAN_POLICY_BUSY', 'EXTERNAL_SCAN_POLICY_UNAVAILABLE'
]);
...
await requireMarkers('src/evidenceRuntime.js', 'Composed evidence runtime', ['createEvidenceVerificationBundleAwareApp']);
await requireMarkers('src/evidence/evidenceVerificationBundleServer.js', 'Portable bundle composition', ['createEvidenceTimeAttestationGovernanceAwareApp']);
await requireMarkers('src/evidence/evidenceTimeAttestationGovernanceServer.js', 'Notary governance composition', ['createEvidenceTimeAttestationAwareApp']);
await requireMarkers('src/evidence/evidenceTimeAttestationServer.js', 'Time-attestation composition', ['createEvidencePreservationAwareApp']);
await requireMarkers('src/evidence/evidencePreservationServer.js', 'Preservation composition', ['createExternalScanAwareApp']);
await requireMarkers('docs/external-scanner-attestations.md', 'External scan runbook', [
  'HMAC-SHA256', 'never auto-release', 'contentSha256', 'RELEASE QUARANTINE', 'sidecar'
]);
