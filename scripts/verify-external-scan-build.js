...
const requiredFiles = [
  'src/evidence/externalScanAttestationRegistry.js', 'src/evidence/externalScanEvidenceRegistry.js',
  'src/evidence/externalScanCallbackHandler.js', 'src/evidence/externalScanManagementHandler.js',
  'src/evidence/externalScanServer.js', 'test/externalScanAttestationRegistry.test.js',
  'test/externalScanHttp.test.js', 'test/externalScanPolicyLock.test.js',
  'test/externalScanHardening.test.js', 'docs/external-scanner-attestations.md',
...
await requireMarkers('src/evidence/externalScanAttestationRegistry.js', 'External scan attestation registry', [
  ...
  'encryptEvidenceJson', 'contentSha256', 'requiredForRelease', 'recordHash', 'ATTESTATION_FIELDS', 'compareAttestationRecency'
]);
await requireMarkers('src/evidence/externalScanEvidenceRegistry.js', 'External scan release gate', [
  ...
  'externalScanAttestations', 'externalScanStatus', 'external-scan-release-policy', '.external-scan-policy-locks',
  "externalScan.mode === 'enforce'", 'external-scanner-disabled'
]);
await requireMarkers('src/evidence/externalScanCallbackHandler.js', 'External scan callback', [
  '/api/workforce-audit/external-scanner/attestations', 'x-content-type-options', 'HMAC realm',
  'external_scan.authentication_failed', 'EXTERNAL_SCAN_POLICY_BUSY', 'EXTERNAL_SCAN_POLICY_UNAVAILABLE'
]);
await requireMarkers('src/evidence/externalScanManagementHandler.js', 'External scan management API', [
  '/api/workforce-audit/external-scanner/status', '/external-scans', 'governance:read', 'authFailure', 'external-scan-management'
]);
...
await requireMarkers('docs/external-scanner-attestations.md', 'External scan runbook', ['HMAC-SHA256', 'never auto-release', 'contentSha256', 'RELEASE QUARANTINE', 'sidecar']);
await requireMarkers('config/evidence-screening.production.env.example', 'Production external scanner configuration', [
  'WORKFORCE_AUDIT_EXTERNAL_SCANNER_MODE=enforce', 'WORKFORCE_AUDIT_EXTERNAL_SCANNER_REQUIRED_FOR_RELEASE=true',
