...
The package signature covers the complete manifest and every sealed-content ciphertext. Altering the purpose, recipient, selected versions, trust records, ciphertext or signature causes offline verification to fail.
...
Rotate `WORKFORCE_AUDIT_EVIDENCE_DISCLOSURE_KEYS` independently from evidence custody, preservation and notary keys. Historical keys must remain available while encrypted receipt indexes reference them.
