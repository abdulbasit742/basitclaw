...
The configured maximum applies to decoded content. The larger JSON transport ceiling accommodates base64 expansion but does not increase the decoded evidence limit.
...
4. the caller has an active JIT grant for `backup:restore` when privileged enforcement is enabled;
5. exact confirmation `DISPOSE EVD-...` is supplied.

The service first commits an encrypted disposition tombstone and custody event, then purges content versions. If physical deletion fails, the item remains disposed with `purgePending=true`; operators must repair storage and complete the purge rather than restoring access to the content.
