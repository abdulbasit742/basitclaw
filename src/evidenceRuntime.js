...
export async function startEvidenceRuntime({
  ...
  try {
    ...
    await new Promise((resolve, reject) => {
      ...
      app.once('error', onError); app.once('listening', onListening); app.listen(port);
    });
    logger.log?.(`BasitClaw listening on http://localhost:${port}`);
    return app;
  } catch (error) { cleanupRefresh(); throw error; }
}
...
if(process.argv[1]===fileURLToPath(import.meta.url)){startEvidenceRuntime().catch((error)=>{console.error('BasitClaw startup failed.',{code:error.code,error:error.message});process.exitCode=1;});}
