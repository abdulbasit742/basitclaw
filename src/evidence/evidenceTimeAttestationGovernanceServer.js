...
export function createEvidenceTimeAttestationGovernanceAwareApp({
  ...
  const server = createServer(async (req, res) => {
    ...
    try {
      ...
      if (governanceHandler.matches(url.pathname)) {
        return await governanceHandler.handle(req, res, requestId);
      }
      return await baseHandler(req, res);
    } catch (error) {
      console.error('Unhandled time-attestation governance server error', {
        requestId,
        code: error?.code,
        error: error?.message
      });
