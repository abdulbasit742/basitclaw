...
import { EvidenceDisclosureIntegrityError, EvidenceDisclosureStoreError } from './evidenceDisclosureStore.js';
...
export function createEvidenceDisclosureHandler({ registry, authenticationGateway, rateLimiter = null, securityTelemetry = null } = {}) {
  ...
  function matches(pathname) {
    return pathname === COLLECTION_ROUTE || pathname === STATUS_ROUTE || ITEM_ROUTE.test(pathname)
      || DOWNLOAD_ROUTE.test(pathname) || VERIFY_ROUTE.test(pathname) || REVOKE_ROUTE.test(pathname);
  }
  ...
  async function handle(req, res, requestId = randomUUID()) {
    ...
    try {
      ...
      const decision = rateLimiter?.consume?.(
        `credential:${principal.keyId ?? principal.subject}:evidence-disclosures`,
        req.method === 'GET' ? 'read' : 'sensitive'
      );
      if (decision) {
      ...
      if (verifyMatch && req.method === 'POST') {
        ...
        return sendJson(res, 200, { success: true, data: registry.verifyEvidenceDisclosure(principal.tenantId, packageId), meta: meta(requestId, principal) }, requestId);
      }
      const revokeMatch = url.pathname.match(REVOKE_ROUTE);
      if (revokeMatch && req.method === 'POST') {
        const packageId = decodeSegment(revokeMatch[1], 'packageId');
      ...
      if (item && req.method === 'GET') {
        ...
        return sendJson(res, 200, { success: true, data: registry.evidenceDisclosureMetadata(principal.tenantId, packageId), meta: meta(requestId, principal) }, requestId);
      }
