...
export function createEvidenceTimeAttestationGovernanceHandler({ registry, authenticationGateway, rateLimiter = null, securityTelemetry = null } = {}) {
  if (!registry || typeof registry.recordTimeAttestationGovernanceEvent !== 'function') throw new TypeError('A time-attestation-governance-aware evidence registry is required.');
  ...
  function matches(pathname) { return pathname === STATUS_ROUTE || pathname === EVENTS_ROUTE || pathname === VERIFY_ROUTE; }
  ...
  async function handle(req, res, requestId = randomUUID()) {
    ...
    try {
      ...
      if (url.pathname === STATUS_ROUTE && req.method === 'GET') {
        return sendJson(res, 200, { success: true, data: registry.evidenceTimeAttestationGovernanceStatus(principal.tenantId), meta: meta(requestId, principal) }, requestId);
      }
      ...
      if (url.pathname === EVENTS_ROUTE && req.method === 'POST') {
        ...
        const data = registry.recordTimeAttestationGovernanceEvent(principal.tenantId, input, { actor: principal.subject });
        record(securityTelemetry, {
          ...
          outcome: data.duplicate ? 'duplicate' : 'success', requestId,
          subject: principal.subject, tenantId: principal.tenantId, method: req.method, route: url.pathname,
          details: {
            eventId: data.event.eventId, eventType: data.event.eventType,
            attestationId: data.event.attestationId, providerId: data.event.providerId,
            keyId: data.event.keyId, effectiveAt: data.event.effectiveAt, retroactive: data.event.retroactive
          }
        });
      ...
      if (url.pathname === VERIFY_ROUTE && req.method === 'POST') {
        ...
        record(securityTelemetry, {
          type: 'evidence_time_attestation_governance.verified', severity: 'info', outcome: 'success', requestId,
          subject: principal.subject, tenantId: principal.tenantId, method: req.method, route: url.pathname,
      ...
      if (error instanceof AuthenticationError) {
        ...
        record(securityTelemetry, { type: 'authentication.failed', severity: 'high', outcome: 'denied', requestId, method: req.method, route: url.pathname, details: { reason: error.code, boundary: 'evidence-notary-governance-journal' } });
        if (failed && !failed.allowed) return rateLimited(res, requestId, failed, 'Too many failed authentication attempts.');
        return sendJson(res, 401, { success: false, error: error.message, code: error.code, meta: { requestId } }, requestId, { 'www-authenticate': challengeHeader(authenticationGateway.mode) });
      }
      ...
      if (error instanceof SecurityControlBusyError) {
        return sendJson(res, 423, {
          success: false, error: 'The time-attestation governance journal is busy. Retry the request.',
          code: 'EVIDENCE_TIME_ATTESTATION_GOVERNANCE_BUSY', details: error.details, meta: meta(requestId, principal)
        }, requestId, { 'retry-after': String(Math.max(1, Math.ceil((error.details?.retryAfterMs ?? 1000) / 1000))) });
      }
      if (error instanceof SecurityControlUnavailableError) return unavailable(res, requestId, error, principal, 'EVIDENCE_TIME_ATTESTATION_GOVERNANCE_STORE_UNAVAILABLE');
...
async function readJson(req, maximumBytes, allowedFields) {
  ...
  if (contentType !== 'application/json') throw new EvidenceValidationError('Time-attestation governance requests require Content-Type application/json.', { field: 'content-type' });
  const chunks = [];
  let size = 0;
  ...
  if (!input || typeof input !== 'object' || Array.isArray(input)) throw new EvidenceValidationError('Time-attestation governance request body must be an object.', { field: 'body' });
  for (const field of Object.keys(input)) if (!allowedFields.has(field)) throw new EvidenceValidationError(`Time-attestation governance request contains unsupported field ${field}.`, { field });
  return input;
}
