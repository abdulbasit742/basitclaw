...
export function createEvidenceDisclosureStore({
  ...
  function create(input, { actor } = {}) {
    const request = validateCreateInput(input, maximumBytes);
    const createdBy = identifier(actor, 'actor');
    const recipient = parseRecipient(request.recipientPublicKeyPem, request.recipientKeyId);
    const payloadBytes = Buffer.from(stableStringify(request.payload), 'utf8');
    if (payloadBytes.length > maximumBytes) {
      throw new EvidenceValidationError('The evidence disclosure payload exceeds the configured package limit.', {
        field: 'payload', maximumPackageBytes: maximumBytes
      });
    }
    ...
    return lock.withLock(`evidence-disclosure:${request.tenantId}`, () => {
      ...
      const packagePath = packageFile(request.tenantId, packageId);
      if (existsSync(packagePath)) {
        throw new EvidenceDisclosureIntegrityError('A disclosure package exists without matching encrypted metadata.', { packageId });
      }

      const dataKey = randomBytes(32);
      const iv = randomBytes(12);
      const cipher = createCipheriv('aes-256-gcm', dataKey, iv);
      const aad = Buffer.from(`${PACKAGE_FORMAT}:${packageId}:${recipient.fingerprint}`);
      cipher.setAAD(aad);
      ...
      const unsigned = {
        ...
        signingKeyId: keyId,
        signingPublicKeyPem: signing.keys.get(keyId).publicKeyPem,
        signingPublicKeyFingerprint: signing.keys.get(keyId).fingerprint,
        payloadSha256,
      ...
      const signature = sign(null, Buffer.from(stableStringify(unsigned)), signing.keys.get(keyId).privateKey).toString('base64');
      const disclosurePackage = { ...unsigned, signature };
      writeJsonExclusive(packagePath, disclosurePackage, packageId);
      ...
      return { created: true, duplicate: false, disclosure: publicMetadata(metadata) };
    });
  }
  ...
  function download(tenantId, packageId) {
    ...
    return lock.withLock(`evidence-disclosure:${tenant}`, () => {
      ...
      assertDeliverable(entry);
      const verified = verifyLocked(tenant, id, index);
      entry.downloadCount += 1;
      entry.lastDownloadedAt = now().toISOString();
      saveIndex(tenant, index);
      return {
        disclosure: publicMetadata(entry),
  ...
  function verifyPackage(tenantId, packageId) {
    ...
    const id = packageIdentifier(packageId);
    return lock.withLock(`evidence-disclosure:${tenant}`, () => {
      const verified = verifyLocked(tenant, id, loadIndex(tenant));
      return { valid: true, disclosure: publicMetadata(verified.metadata), package: packageSummary(verified.package) };
    });
  }
  ...
  function tenantStatus(tenantId) {
    ...
    try {
      ...
      for (const entry of index.packages) {
        if (entry.revokedAt) revoked += 1;
        else if (current >= new Date(entry.expiresAt)) expired += 1;
        else if (entry.downloadCount >= entry.maximumDownloads) exhausted += 1;
        else active += 1;
      }
      return { status: 'ready', enabled: true, total: index.packages.length, active, expired, revoked, exhausted };
    } catch (error) {
  ...
  function health() {
    try {
      ...
      return {
        status: 'ready',
        enabled: true,
        mode: 'recipient-bound-encrypted-disclosures',
        encryptedMetadata: true,
        recipientEncryptedPackages: true,
        signedPackages: true,
        generalEvidenceDownload: false,
        maximumPackageBytes: maximumBytes,
        signingKeyId: signing.primaryKeyId,
        signingPublicKeyFingerprint: signing.keys.get(signing.primaryKeyId).fingerprint,
        mutex: lock.health()
      };
    } catch (error) {
      return { status: 'unavailable', enabled: true, error: error?.code ?? 'evidence_disclosure_store_unavailable' };
  ...
  function verifyLocked(tenant, packageId, index) {
    ...
    let disclosurePackage;
    try { disclosurePackage = JSON.parse(readFileSync(path, 'utf8')); }
    catch (error) { throw new EvidenceDisclosureStoreError('The disclosure package cannot be read.', { packageId }, error); }
    if (!disclosurePackage || disclosurePackage.format !== PACKAGE_FORMAT || disclosurePackage.version !== 1
        || disclosurePackage.packageId !== packageId) {
      throw new EvidenceDisclosureIntegrityError('The disclosure package identity is invalid.', { packageId });
    }
    const configured = signing.keys.get(disclosurePackage.signingKeyId);
    if (!configured || configured.fingerprint !== disclosurePackage.signingPublicKeyFingerprint
        || configured.publicKeyPem !== disclosurePackage.signingPublicKeyPem) {
      throw new EvidenceDisclosureIntegrityError('The disclosure package references an untrusted signing key.', {
        packageId, signingKeyId: disclosurePackage.signingKeyId
      });
    }
    const { signature, ...unsigned } = disclosurePackage;
    let valid = false;
    try {
      valid = verify(null, Buffer.from(stableStringify(unsigned)), configured.publicKey, Buffer.from(signature, 'base64'));
    } catch (error) {
      throw new EvidenceDisclosureIntegrityError('The disclosure package signature is malformed.', { packageId }, error);
    }
    if (!valid) throw new EvidenceDisclosureIntegrityError('The disclosure package signature verification failed.', { packageId });
    const packageSha256 = sha256(Buffer.from(stableStringify(disclosurePackage)));
    if (packageSha256 !== entry.packageSha256 || disclosurePackage.payloadSha256 !== entry.payloadSha256
        || disclosurePackage.payloadSizeBytes !== entry.payloadSizeBytes
        || disclosurePackage.recipientKeyFingerprint !== entry.recipientKeyFingerprint
        || disclosurePackage.expiresAt !== entry.expiresAt
        || disclosurePackage.maximumDownloads !== entry.maximumDownloads) {
      throw new EvidenceDisclosureIntegrityError('The disclosure package does not match its encrypted metadata.', { packageId });
    }
    return { metadata: entry, package: disclosurePackage };
  }

  function assertDeliverable(entry) {
    ...
    if (entry.downloadCount >= entry.maximumDownloads) {
      throw new EvidenceDisclosureLimitError(entry.packageId, entry.maximumDownloads);
    }
  }
  ...
  function saveIndex(tenant, index) {
    try {
      atomicWriteEvidenceJson(indexFile(tenant), encryptEvidenceJson(index, metadataKeyring, indexAad(tenant)));
    } catch (error) {
      if (error instanceof EvidenceDisclosureIntegrityError) throw error;
      throw new EvidenceDisclosureStoreError('The encrypted disclosure index could not be committed.', {}, error);
    }
  }
...
export function createEvidenceDisclosureStoreFromEnvironment({ env = process.env, evidenceRegistry } = {}) {
  ...
  if (!evidenceRegistry?.evidencePreservationEnabled || !evidenceRegistry?.evidenceTimeAttestationEnabled) {
    throw new EvidenceDisclosureStoreError('Evidence disclosure packages require preservation and time-attestation controls.', {
  ...
  if (!rawKeys || !primaryKeyId) {
    throw new EvidenceDisclosureStoreError('Dedicated evidence disclosure metadata keys are required.', {
      reason: 'missing_disclosure_metadata_keys'
    });
  }
  if (!rawSigning || !primarySigningKeyId) {
    throw new EvidenceDisclosureStoreError('Dedicated evidence disclosure signing keys are required.', {
      reason: 'missing_disclosure_signing_keys'
    });
  }
  try {
    ...
    throw new EvidenceDisclosureStoreError('Evidence disclosure configuration is invalid.', {
      reason: error?.code ?? 'invalid_disclosure_configuration'
    }, error);
  }
}

function validateCreateInput(input, maximumBytes) {
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new EvidenceValidationError('A valid evidence disclosure package is required.');
  }
  const tenantId = identifier(input.tenantId, 'tenantId');
  const recipientKeyId = identifier(input.recipientKeyId, 'recipientKeyId');
  const recipientPublicKeyPem = cleanText(input.recipientPublicKeyPem, 'recipientPublicKeyPem', 100, 20_000);
  const expiresAt = isoDate(input.expiresAt, 'expiresAt');
  if (new Date(expiresAt) <= new Date()) throw new EvidenceValidationError('expiresAt must be in the future.', { field: 'expiresAt' });
  const maximumDownloads = integer(input.maximumDownloads, 'maximumDownloads', 1, 1000);
  const purpose = cleanText(input.purpose, 'purpose', 10, 500);
  const itemCount = integer(input.itemCount, 'itemCount', 1, 1000);
  if (!input.payload || typeof input.payload !== 'object' || Array.isArray(input.payload)) {
    throw new EvidenceValidationError('payload must be an object.', { field: 'payload' });
  }
  if (Buffer.byteLength(stableStringify(input.payload)) > maximumBytes) {
    throw new EvidenceValidationError('payload exceeds the configured package limit.', { field: 'payload' });
  }
  return { tenantId, recipientKeyId, recipientPublicKeyPem, expiresAt, maximumDownloads, purpose, itemCount, payload: input.payload };
}

function parseRecipient(pem, keyId) {
  try {
    const publicKey = createPublicKey(pem);
    if (publicKey.asymmetricKeyType !== 'rsa' && publicKey.asymmetricKeyType !== 'rsa-pss') {
      throw new Error('recipient_key_not_rsa');
    }
    const bits = publicKey.asymmetricKeyDetails?.modulusLength ?? 0;
    if (bits < 2048) throw new Error('recipient_key_too_small');
    const canonical = publicKey.export({ type: 'spki', format: 'pem' }).toString();
    return { publicKey, keyId, fingerprint: sha256(canonical), canonical };
  } catch (error) {
    throw new EvidenceValidationError('recipientPublicKeyPem must contain an RSA public key of at least 2048 bits.', {
      field: 'recipientPublicKeyPem', reason: error.message
    });
  }
}
...
function parseSigningKeys(raw, primaryKeyId) {
  ...
  const primary = String(primaryKeyId ?? '');
  if (!keys.has(primary)) throw new TypeError('The disclosure primary signing key ID is not present in the keyring.');
  return Object.freeze({ keys, primaryKeyId: primary });
}

function disclosureId(input) {
  return `DSP-${sha256(stableStringify(input)).slice(0, 32)}`;
}
function findMetadata(index, packageId) {
  const entry = index.packages.find((candidate) => candidate.packageId === packageId);
  if (!entry) throw new EvidenceDisclosureIntegrityError('The disclosure package metadata does not exist.', { packageId });
  return entry;
}
function publicMetadata(entry) {
  return {
    ...
    maximumDownloads: entry.maximumDownloads,
    ...
    signingPublicKeyFingerprint: entry.signingPublicKeyFingerprint,
...
function packageSummary(value) {
  return {
    ...
    recipientKeyFingerprint: value.recipientKeyFingerprint,
    payloadSha256: value.payloadSha256,
    payloadSizeBytes: value.payloadSizeBytes,
    signingKeyId: value.signingKeyId,
    signingPublicKeyFingerprint: value.signingPublicKeyFingerprint
  };
}
function writeJsonExclusive(path, value, packageId) {
  ...
  try {
    ...
    if (created && !committed) {
      try { /* A partial file created by this invocation may be removed by deployment repair tooling. */ } catch {}
    }
...
function disabledStore() {
  ...
  return Object.freeze({
    ...
    list() { return []; },
    metadata() { throw new EvidenceConflictError('Evidence disclosure packages are disabled.'); },
