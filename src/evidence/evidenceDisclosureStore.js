...
const FILTER_STATES = new Set(['pending', 'packaged', 'rejected', 'revoked', 'expired']);
...
export function createEvidenceDisclosureStore({
  ...
  function createRequest(input, context = {}) {
    const request = normaliseCreate(input, context, { itemLimit, ttlLimitHours, now });
    const recipient = parseRecipientKey(request.recipientPublicKeyPem);
    const selections = request.evidence.map((selection) => {
      const resolved = normaliseResolved(resolveEvidence(request.tenantId, selection), selection);
      return publicSelection(resolved);
    });
    ...
    return lock.withLock(`evidence-disclosures:${request.tenantId}`, () => {
      ...
      if (duplicate) return { created: false, duplicate: true, request: publicRequest(duplicate, now(), approvalQuorum) };
      const record = {
        requestId,
        tenantId: request.tenantId,
        state: 'pending',
      ...
      appendEvent(index, {
        type: 'disclosure.requested', requestId, actor: request.requestedBy,
      ...
      return { created: true, duplicate: false, request: publicRequest(record, now(), approvalQuorum) };
    });
  }
  ...
  function approve(tenantId, requestId, input = {}, context = {}) {
    ...
    requireConfirmation(input.confirmation, `APPROVE DISCLOSURE ${id}`);
    ...
    return lock.withLock(`evidence-disclosures:${tenant}`, () => {
      ...
      if (record.approvals.some((approval) => approval.actor === actor)) {
        throw new EvidenceDisclosureApprovalError('The approver has already approved this request.', {
          requestId: id,
      ...
      appendEvent(index, {
        type: 'disclosure.approved', requestId: id, actor, occurredAt: approvedAt,
      ...
      return { approved: true, packaged, request: publicRequest(record, now(), approvalQuorum) };
    });
  }
  ...
  function terminalDecision(decision, tenantId, requestId, input, context) {
    ...
    const actor = identifier(context.actor, 'actor');
    const reason = cleanText(input.reason, 'reason', 10, 500);
    const verb = decision === 'rejected' ? 'REJECT' : 'REVOKE';
    requireConfirmation(input.confirmation, `${verb} DISCLOSURE ${id}`);
    ...
    return lock.withLock(`evidence-disclosures:${tenant}`, () => {
      ...
      if (decision === 'rejected') {
        ...
        if (!['pending', 'packaged'].includes(state)) {
          throw new EvidenceConflictError('Only pending or packaged disclosures can be revoked.', {
            requestId: id,
            state
          });
        }
      ...
      saveIndex(tenant, index);
      return { updated: true, request: publicRequest(record, now(), approvalQuorum) };
    });
  }

  function list(tenantId, { state = null, limit = 200 } = {}) {
    const tenant = identifier(tenantId, 'tenantId');
    const stateFilter = state === null ? null : enumValue(state, FILTER_STATES, 'state');
    return loadSafe(tenant).requests
      .filter((record) => !stateFilter || effectiveState(record, now()) === stateFilter)
      .sort((left, right) => right.requestedAt.localeCompare(left.requestedAt))
      .slice(0, integer(limit, 'limit', 1, 2000))
      .map((record) => publicRequest(record, now(), approvalQuorum));
  }

  function get(tenantId, requestId) {
    const tenant = identifier(tenantId, 'tenantId');
    return publicRequest(findRequest(loadSafe(tenant), requestIdentifier(requestId)), now(), approvalQuorum);
  }
  ...
  function sealedPackage(tenantId, requestId) {
    ...
    const state = effectiveState(record, now());
    if (state !== 'packaged') {
      throw new EvidenceConflictError('The recipient-sealed disclosure package is not available.', {
    ...
    if (digest !== record.packageEnvelopeSha256) {
      throw new EvidenceDisclosureIntegrityError('The disclosure package does not match its request record.', {
        requestId: record.requestId,
  ...
  function events(tenantId, requestId = null, { limit = 500 } = {}) {
    ...
    return loadSafe(tenant).events
      .filter((event) => !id || event.requestId === id)
  ...
  function verifyTenant(tenantId) {
    ...
    for (const record of index.requests) {
      ...
      const envelope = readPackage(tenant, record.packageId);
      if (packageDigest(envelope) !== record.packageEnvelopeSha256) {
        throw new EvidenceDisclosureIntegrityError('A disclosure package digest is invalid.', {
          requestId: record.requestId,
          packageId: record.packageId
        });
      }
  ...
  function tenantStatus(tenantId) {
    ...
    try {
      ...
      return {
        ...
        enabled: true,
        minimumApprovers: approvalQuorum,
        error: error?.code ?? 'evidence_disclosure_store_unavailable'
      };
    }
  }
  ...
  function health() {
    try {
      ...
      return {
        status: 'ready',
        enabled: true,
        mode: 'shared-file-governed-disclosure',
        encryptedRecords: true,
        recipientSealedPackages: true,
        requesterApprovalSeparation: true,
        minimumApprovers: approvalQuorum,
        maximumEvidenceItems: itemLimit,
        maximumPackageBytes: byteLimit,
        maximumTtlHours: ttlLimitHours,
  ...
  function packageRecord(record) {
    ...
    if (existsSync(path)) {
      ...
      if (envelope.requestId !== record.requestId
          || envelope.recipientKeyFingerprint !== record.recipientKeyFingerprint) {
        throw new EvidenceDisclosureIntegrityError('A conflicting recipient-sealed package already exists.', {
          requestId: record.requestId,
          packageId
        });
      }
      ...
      if (totalBytes > byteLimit) {
        throw new EvidenceValidationError('The disclosure exceeds the configured package byte limit.', {
          field: 'evidence',
          totalBytes,
          maximumPackageBytes: byteLimit
        });
      ...
      const manifest = {
        ...
        approvedBy: record.approvals.map((approval) => ({ actor: approval.actor, approvedAt: approval.approvedAt })),
      ...
      const plaintext = Buffer.from(stableStringify(manifest), 'utf8');
      const contentKey = randomBytes(32);
      const iv = randomBytes(12);
      const aad = Buffer.from(`basitclaw:evidence-disclosure:${packageId}:${record.requestId}`, 'utf8');
    ...
    record.packageEnvelopeSha256 = packageDigest(envelope);
  ...
  function loadIndex(tenant) {
    ...
    try {
      ...
      throw new EvidenceDisclosureStoreError('The evidence disclosure index is unreadable.', {}, error);
    }
    ...
    if (!index || index.format !== INDEX_FORMAT || index.version !== 1
        || index.tenantId !== tenant || !Array.isArray(index.requests)
  ...
  function loadSafe(tenant) {
    try {
      ...
      if (error instanceof EvidenceDisclosureStoreError
          || error instanceof EvidenceDisclosureIntegrityError) throw error;
  ...
  function saveIndex(tenant, index) {
    ...
    atomicWriteEvidenceJson(
      indexPath(tenant),
      encryptEvidenceJson(index, encryption, indexAad(tenant))
    );
  }
  ...
  function packagePath(tenant, packageId) {
    const directoryPath = resolve(tenantEvidenceDirectory(root, tenant), 'packages');
    mkdirSync(directoryPath, { recursive: true, mode: 0o700 });
    return resolve(directoryPath, `${packageId}.sealed`);
  }
  ...
  function readPackage(tenant, packageId) {
    ...
    let envelope;
    try {
      envelope = JSON.parse(readFileSync(packagePath(tenant, id), 'utf8'));
    } catch (error) {
      throw new EvidenceDisclosureStoreError('The recipient-sealed disclosure package is unreadable.', { packageId: id }, error);
    }
    if (!envelope || envelope.format !== PACKAGE_FORMAT || envelope.version !== 1
        || envelope.packageId !== id || !REQUEST_ID.test(envelope.requestId)
        || envelope.algorithm !== PACKAGE_ALGORITHM) {
...
export function createEvidenceDisclosureStoreFromEnvironment({ env = process.env, resolveEvidence } = {}) {
  try {
    ...
    if (!rawKeys || !primaryKeyId) {
      throw new EvidenceDisclosureStoreError('Dedicated disclosure keys and primary key ID are required.', {
...
function normaliseCreate(input, context, { itemLimit, ttlLimitHours, now }) {
  ...
  const ttlMs = new Date(expiresAt).getTime() - new Date(requestedAt).getTime();
  if (ttlMs <= 0 || ttlMs > ttlLimitHours * 3_600_000) {
  ...
  const seen = new Set();
  const evidence = input.evidence.map((selection, index) => {
    ...
    const version = integer(selection.version, `evidence[${index}].version`, 1, 1_000_000);
...
function normaliseResolved(input, selection) {
  ...
  return {
    ...
    preservationReceiptSha256: input.preservationReceiptSha256
      ? hashValue(input.preservationReceiptSha256, 'preservationReceiptSha256')
      : null,
...
function parseRecipientKey(value) {
  ...
  try {
    ...
    throw new EvidenceValidationError('recipientPublicKeyPem must contain a valid public key.', {
      field: 'recipientPublicKeyPem'
    }, error);
  }
  if (!['rsa', 'rsa-pss'].includes(publicKey.asymmetricKeyType)) {
    throw new EvidenceValidationError('recipientPublicKeyPem must be an RSA public key.', {
      field: 'recipientPublicKeyPem'
    });
  }
  if ((publicKey.asymmetricKeyDetails?.modulusLength ?? 0) < 2048) {
    throw new EvidenceValidationError('recipientPublicKeyPem must be at least 2048 bits.', {
      field: 'recipientPublicKeyPem'
    });
  }
...
function publicRequest(record, date, minimumApprovers) {
  return {
    requestId: record.requestId,
    state: effectiveState(record, date),
    requestedBy: record.requestedBy,
    requestedAt: record.requestedAt,
    expiresAt: record.expiresAt,
    recipientId: record.recipientId,
    ...
    caseReference: record.caseReference,
    purpose: record.purpose,
    evidence: record.evidence.map((selection) => ({ ...selection })),
...
function effectiveState(record, date) {
  if (['rejected', 'revoked'].includes(record.state)) return record.state;
  if (new Date(record.expiresAt) <= date) return 'expired';
  return record.state;
}

function findRequest(index, requestId) {
  const record = index.requests.find((entry) => entry.requestId === requestId);
  if (!record) throw new EvidenceValidationError('The evidence disclosure request was not found.', { requestId });
  return record;
}

function assertPending(record, date) {
  const state = effectiveState(record, date);
  if (state !== 'pending') {
    throw new EvidenceConflictError('The evidence disclosure request is not pending.', {
      requestId: record.requestId,
      state
    });
  }
...
function packageDigest(envelope) {
  return sha256(Buffer.from(stableStringify(envelope), 'utf8'));
}
...
function writeJsonExclusive(path, value, packageId) {
  ...
  try {
    ...
    if (error?.code === 'EEXIST') {
      throw new EvidenceDisclosureIntegrityError('A conflicting recipient-sealed disclosure package already exists.', { packageId });
    }
    throw new EvidenceDisclosureStoreError('The recipient-sealed disclosure package could not be committed.', { packageId }, error);
  }
}

function indexAad(tenantId) {
  return `basitclaw:evidence-disclosures:${tenantId}`;
}
...
function packageIdentifier(value) {
  const id = String(value ?? '');
  if (!PACKAGE_ID.test(id)) throw new EvidenceValidationError('packageId is invalid.', { field: 'packageId' });
  return id;
}
...
function isoDate(value, field) {
  ...
  if (Number.isNaN(date.getTime())) throw new EvidenceValidationError(`${field} must be a valid ISO date.`, { field });
