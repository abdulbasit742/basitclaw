...
export function createEvidenceAssuranceBundleStore({
  ...
  function queue(input) {
    const source = validateQueueInput(input, recipientMap, now);
    ...
    return lock.withLock(`assurance-bundles:${source.recipientId}`, () => {
      ...
      if (existsSync(path)) {
        ...
        return { duplicate: true, bundle: publicRecord(existing) };
      }
      ...
      return { duplicate: false, bundle: publicRecord(record) };
    });
  }
    ...
    for (const recipientId of recipientMap.keys()) {
      lock.withLock(`assurance-bundles:${recipientId}`, () => {
        ...
        for (const filename of bundleNames(recipientId)) {
          const record = readRecord(bundlePath(recipientId, filename.slice(0, -7)), recipientId, filename.slice(0, -7));
    ...
    return lock.withLock(`assurance-bundles:${auth.recipientId}`, () => {
      ...
      for (const filename of bundleNames(auth.recipientId)) {
        ...
        if (!selected.length && estimate > claimBytes) {
          throw new EvidenceValidationError('The sealed assurance bundle exceeds the configured claim response limit.', {
            bundleId,
        ...
        selected.push({
          ...
          sealedPackage: record.sealedPackage
        });
      }
      return { recipientId: auth.recipientId, jobs: selected };
    });
  }
  ...
  function acknowledgeSigned(bundleId, bodyBytes, headers) {
    ...
    return lock.withLock(`assurance-bundles:${auth.recipientId}`, () => {
      ...
      if (!safeEqual(tokenHash, record.claimTokenHash)
          || String(input.packageSha256 ?? '') !== record.packageSha256) {
  ...
  function tenantStatus(tenantId) {
    ...
    return {
      status: counts.expired ? 'attention' : 'ready',
      enabled: true,
      required: isRequired,
      total: rows.length,
      ...counts
  ...
  function authenticateSigned(bodyBytes, headers, operation) {
    ...
    const supplied = header(headers, 'x-basitclaw-recipient-signature');
    ...
    if (Math.abs(now().getTime() - timestampMs) > skewMs) {
    ...
    let signature;
    try { signature = strictBase64(supplied, 'recipient signature'); }
    catch { throw new EvidenceAssuranceBundleAuthenticationError(); }
    if (signature.length !== expected.length || !timingSafeEqual(signature, expected)) {
      throw new EvidenceAssuranceBundleAuthenticationError();
    }
  ...
  function registerReplay(auth) {
    ...
    try {
      ...
      if (error?.code === 'EEXIST') {
        throw new EvidenceAssuranceBundleAuthenticationError('The assurance recipient request was already used.', { reason: 'replay' });
      }
  ...
  function pruneLocked(recipientId) {
    const rows = bundleNames(recipientId).map((filename) => {
      const id = filename.slice(0, -7);
      return { id, path: bundlePath(recipientId, id), record: readRecord(bundlePath(recipientId, id), recipientId, id) };
    });
    const terminal = rows.filter((row) => ['delivered', 'expired'].includes(row.record.state))
      .sort((a, b) => String(a.record.deliveredAt ?? a.record.expiresAt).localeCompare(String(b.record.deliveredAt ?? b.record.expiresAt)));
    while (terminal.length > retained) {
  ...
  function readRecord(path, recipientId, bundleId) {
    try {
      ...
      if (!record || record.format !== FORMAT || record.version !== 1 || record.bundleId !== bundleId || record.recipientId !== recipientId) {
        throw new EvidenceIntegrityError('The assurance bundle record identity is invalid.', { bundleId });
      }
      if (record.sealedPackage && sha256(stableStringify(record.sealedPackage)) !== record.packageSha256) {
        throw new EvidenceIntegrityError('The assurance bundle package digest does not match its record.', { bundleId });
      }
      return record;
    } catch (error) {
  ...
  function bundleNames(recipientId) {
    return readdirSync(recipientRoot(recipientId)).filter((name) => name.endsWith('.bundle') && BUNDLE_ID.test(name.slice(0, -7))).sort();
  }
...
export function createEvidenceAssuranceBundleStoreFromEnvironment({ env = process.env } = {}) {
  ...
  try {
    return createEvidenceAssuranceBundleStore({
      ...
      claimLeaseMs: envValue(env.WORKFORCE_AUDIT_ASSURANCE_BUNDLE_CLAIM_LEASE_MS) ?? CLAIM_LEASE_MS,
...
function sealForRecipient(payload, recipient, bundleId) {
  ...
  const { createCipheriv } = awaitImportCrypto();
...
function awaitImportCrypto() {
  // Kept synchronous and dependency-free; isolated for test instrumentation.
  return { createCipheriv: globalThis.__basitclawCreateCipheriv ?? ((...args) => requireCryptoCreateCipheriv(...args)) };
}

function requireCryptoCreateCipheriv(...args) {
  // Dynamic CommonJS is unavailable in ESM, so this function is replaced below at module initialisation.
  return createCipherivDirect(...args);
}

import { createCipheriv as createCipherivDirect } from 'node:crypto';

function validateQueueInput(input, recipients, now) {
  ...
  const expiresAt = new Date(now().getTime() + (input.ttlMs ?? 86_400_000)).toISOString();
  return { tenantId, evidenceId, evidenceVersion, contentSha256, recipientId, requestedBy, purpose, manifest: structuredClone(input.manifest), evidence: structuredClone(input.evidence), createdAt, expiresAt };
}
...
function bundleIdFor(input) {
  return `ASB-${sha256([input.tenantId, input.evidenceId, String(input.evidenceVersion), input.contentSha256, input.recipientId, input.manifest.bundleDigest].join('|')).slice(0, 32)}`;
}
function assertSameBundle(record, input) {
  if (record.tenantId !== input.tenantId || record.evidenceId !== input.evidenceId || record.evidenceVersion !== input.evidenceVersion
      || record.contentSha256 !== input.contentSha256 || record.recipientId !== input.recipientId) {
    throw new EvidenceIntegrityError('An existing assurance bundle conflicts with this request.', { bundleId: record.bundleId });
  }
}
function publicRecord(record) {
  return {
    ...
    recipientPublicKeyId: record.recipientPublicKeyId,
...
function disabledStore() {
  ...
  return Object.freeze({
    mode: 'disabled', enabled: false, required: false,
    queue() { throw new EvidenceConflictError('Assurance bundle delivery is disabled.'); },
    list() { return []; },
    claimSigned() { throw new EvidenceConflictError('Assurance bundle delivery is disabled.'); },
    acknowledgeSigned() { throw new EvidenceConflictError('Assurance bundle delivery is disabled.'); },
    tenantStatus() { return status; }, health() { return status; }, recipientIds() { return []; }
  });
}
