...
export function createEvidenceDisclosurePackageRegistry({
  ...
  function buildManifest(tenantId, item, selectedVersions) {
    ...
    const timeAttestations = typeof registry.evidenceTimeAttestations === 'function'
      ? preservations.map((receipt) => ({
          archiveId: receipt.archiveId,
          evidenceVersion: receipt.evidenceVersion,
          attestations: registry.evidenceTimeAttestations(tenantId, receipt.archiveId, { limit: 5000 })
        }))
      : [];
