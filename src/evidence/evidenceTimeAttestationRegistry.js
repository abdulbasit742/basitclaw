...
export function createEvidenceTimeAttestationRegistry({
  ...
  function evidenceTimeAttestationStatus(tenantId) {
    ...
    for (const item of items) {
      ...
      for (const version of item.versions ?? []) {
        ...
        preservedVersions += 1;
        const quorum = timeAttestations.quorumForArchive(tenantId, receipt.archiveId);
        if (quorum) quorumVersions += 1;
        else missingQuorum.push({ evidenceId: item.evidenceId, version: version.version, archiveId: receipt.archiveId });
      }
    }
    return {
      ...base,
  ...
  function dispose(tenantId, evidenceId, input, context = {}) {
    if (timeAttestations.requiredForDisposition) {
      ...
      for (const version of item.versions ?? []) {
        ...
        if (!receipt) {
          missingPreservations.push(version.version);
          continue;
        }
        if (!timeAttestations.quorumForArchive(tenantId, receipt.archiveId)) {
          missingQuorum.push({ version: version.version, archiveId: receipt.archiveId });
        }
      }
  ...
  function verify(tenantId, evidenceId = null) {
    ...
    const results = receipts.map((receipt) => timeAttestations.verifyArchive(tenantId, receipt.archiveId));
    return {
      ...base,
      timeAttestations: {
        valid: true,
...
function challengeFromVerification(tenantId, verified) {
  ...
  return {
    ...
    receiptSha256: sha256(stableStringify(receipt)),
