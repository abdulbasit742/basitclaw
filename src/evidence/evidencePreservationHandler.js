...
const STATUS_ROUTE = '/api/workforce-audit/evidence-preservation/status';
const EVIDENCE_ROUTE = /^\/api\/workforce-audit\/evidence\/([^/]+)\/preservations$/;
const VERIFY_ROUTE = /^\/api\/workforce-audit\/evidence-preservation\/([^/]+)\/verify$/;

export function createEvidencePreservationHandler({
  registry,
  authenticationGateway,
  rateLimiter = null,
  securityTelemetry = null
} = {}) {
  if (!registry || typeof registry.preserveEvidence !== 'function') {
  ...
  function matches(pathname) {
    return pathname === STATUS_ROUTE || EVIDENCE_ROUTE.test(pathname) || VERIFY_ROUTE.test(pathname);
  }
  ...
  async function handle(req, res, requestId = randomUUID()) {
    ...
    try {
      ...
      authenticationGateway.authorise(principal, write ? 'evidence:preserve' : 'governance:read');
      const decision = rateLimiter?.consume?.(
        `credential:${principal.keyId ?? principal.subject}:evidence-preservation`,
        write ? 'write' : 'read'
      );
      if (decision) {
        applyRateHeaders(res, rateLimiter, decision);
        if (!decision.allowed) return rateLimited(res, requestId, decision, 'The evidence preservation request rate limit has been exceeded.');
      }

      if (url.pathname === STATUS_ROUTE && req.method === 'GET') {
        return sendJson(res, 200, {
          success: true,
          data: registry.evidencePreservationStatus(principal.tenantId),
      ...
      if (evidenceMatch) {
        ...
        if (req.method === 'GET') {
          const data = registry.evidencePreservationReceipts(principal.tenantId, evidenceId, {
        ...
        if (req.method === 'POST') {
          const input = await readJson(req, 16_384, new Set(['version', 'purpose', 'confirmation']));
          const data = registry.preserveEvidence(principal.tenantId, evidenceId, input, { actor: principal.subject });
          record(securityTelemetry, {
            type: data.duplicate ? 'evidence.preservation_duplicate' : 'evidence.preserved',
            severity: 'info',
            ...
            route: url.pathname,
            details: {
              archiveId: data.receipt.archiveId,
              evidenceId,
              version: data.receipt.evidenceVersion,
              retentionUntil: data.receipt.retentionUntil
            }
          });
      ...
      if (verifyMatch && req.method === 'POST') {
        const archiveId = decodeSegment(verifyMatch[1], 'archiveId');
        const data = registry.verifyEvidencePreservation(principal.tenantId, archiveId);
        record(securityTelemetry, {
          type: 'evidence.preservation_verified',
          severity: 'info',
          outcome: 'success',
          requestId,
          subject: principal.subject,
      ...
      if (error instanceof AuthenticationError) {
        ...
        record(securityTelemetry, {
          ...
          details: { reason: error.code, boundary: 'evidence-preservation' }
        });
      ...
      if (error instanceof AuthorizationError) {
        record(securityTelemetry, {
          ...
          method: req.method, route: url.pathname,
          details: { reason: error.details?.reason, boundary: 'evidence-preservation' }
        });
      ...
      if (error instanceof SecurityControlBusyError) {
        return sendJson(res, 423, {
          success: false,
          error: 'The evidence preservation boundary is busy. Retry the request.',
          code: 'EVIDENCE_PRESERVATION_BUSY',
      ...
      if (error instanceof SecurityControlUnavailableError) {
        return unavailable(res, requestId, error, principal, 'EVIDENCE_PRESERVATION_STORE_UNAVAILABLE');
      }
      if (error instanceof EvidenceValidationError || error instanceof EvidenceNotFoundError
          ...
          || error instanceof EvidenceStoreError || error instanceof EvidencePreservationStoreError
          || error instanceof EvidencePreservationIntegrityError) {
        record(securityTelemetry, {
          type: 'evidence.preservation_denied',
...
async function readJson(req, maximumBytes, allowed) {
  ...
  if (contentType !== 'application/json') {
    throw new EvidenceValidationError('Evidence preservation requests require Content-Type application/json.', { field: 'content-type' });
  }
  const chunks = [];
  let size = 0;
  for await (const chunk of req) {
    size += chunk.length;
    if (size > maximumBytes) throw new EvidenceValidationError('Evidence preservation request body is too large.', { field: 'body' });
    chunks.push(chunk);
  }
  let input;
  try { input = JSON.parse(Buffer.concat(chunks).toString('utf8') || '{}'); }
  catch { throw new EvidenceValidationError('Evidence preservation request body must be valid JSON.', { field: 'body' }); }
  if (!input || typeof input !== 'object' || Array.isArray(input)) {
    throw new EvidenceValidationError('Evidence preservation request body must be an object.', { field: 'body' });
  }
  for (const key of Object.keys(input)) {
    if (!allowed.has(key)) throw new EvidenceValidationError(`Evidence preservation request contains unsupported field ${key}.`, { field: key });
  }
  return input;
}
...
function decodeSegment(value, field) {
  ...
  catch { throw new EvidenceValidationError(`${field} is not valid percent-encoded text.`, { field }); }
}
function positiveInteger(value, fallback, maximum) {
  if (value === null) return fallback;
  const parsed = Number(value);
  if (!Number.isInteger(parsed) || parsed < 1) throw new EvidenceValidationError('limit must be a positive integer.', { field: 'limit' });
...
function record(telemetry, input) { try { telemetry?.record?.(input); } catch (error) { console.error('Evidence preservation telemetry failed', error); } }
function notFound(res, requestId) { return sendJson(res, 404, { success: false, error: 'Evidence preservation route not found.', code: 'NOT_FOUND', meta: { requestId } }, requestId); }
