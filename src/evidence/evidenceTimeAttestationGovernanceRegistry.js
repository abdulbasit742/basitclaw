...
export function createEvidenceTimeAttestationGovernanceRegistry({
  ...
  function recordTimeAttestation(input) {
    ...
    return { ...result, governance: publicDecision(evaluation.decisions.get(result.attestation.attestationId)) };
  }
  ...
  function recordTimeAttestationGovernanceEvent(tenantId, input, context = {}) {
    ...
    if (!input || typeof input !== 'object' || Array.isArray(input)) throw new EvidenceValidationError('A valid time-attestation governance request is required.');
  ...
  function validateGovernanceTarget(tenantId, input) {
    if (input.eventType !== 'attestation_revoked' && input.eventType !== 'attestation_superseded') return;
    ...
    if (!records.some((record) => record.attestationId === input.attestationId)) {
      throw new EvidenceValidationError('The governed time attestation does not exist for the archive.', { field: 'attestationId', attestationId: input.attestationId });
    }
    if (input.eventType === 'attestation_superseded'
        && !records.some((record) => record.attestationId === input.replacementAttestationId)) {
      throw new EvidenceValidationError('The replacement time attestation does not exist for the archive.', { field: 'replacementAttestationId', replacementAttestationId: input.replacementAttestationId });
    }
  }

  function assertConfirmation(input) {
    let expected;
    if (input.eventType === 'attestation_revoked') expected = `REVOKE ATTESTATION ${input.attestationId}`;
    else if (input.eventType === 'attestation_superseded') expected = `SUPERSEDE ATTESTATION ${input.attestationId} WITH ${input.replacementAttestationId}`;
    else if (input.eventType === 'provider_revoked') expected = `REVOKE NOTARY PROVIDER ${input.providerId}`;
    else if (input.eventType === 'key_revoked') expected = `REVOKE NOTARY KEY ${input.providerId}/${input.keyId}`;
    else throw new EvidenceValidationError('eventType is invalid.', { field: 'eventType' });
    if (input.confirmation !== expected) throw new EvidenceValidationError(`confirmation must be exactly ${expected}.`, { field: 'confirmation' });
  }

  function evidenceTimeAttestationGovernanceEvents(tenantId, options = {}) { return governance.list(tenantId, options); }
  function verifyEvidenceTimeAttestationGovernance(tenantId) { return governance.verifyTenant(tenantId); }
  ...
  function effectiveArchiveVerification(tenantId, archiveId, { at = undefined } = {}) {
    ...
    return {
      ...
      attestationDecisions: attestations.map((attestation) => ({ ...attestation, governance: publicDecision(evaluation.decisions.get(attestation.attestationId)) }))
    };
  }

  function verifyEvidenceTimeAttestations(tenantId, archiveId) { return effectiveArchiveVerification(tenantId, archiveId); }

  function currentOperationalPosture(tenantId, evidenceId = null) {
    const items = evidenceId ? [registry.get(tenantId, evidenceId)] : registry.list(tenantId, { limit: 5000 });
    ...
    for (const item of items) {
      ...
      for (const version of item.versions ?? []) {
        ...
        else missingOperationalQuorum.push({
    ...
    return { totalVersions, preservedVersions, operationalQuorumVersions, missingPreservations, missingOperationalQuorum };
  }
  ...
  function tenantStatus(tenantId) {
    ...
    try {
      ...
      return {
        ...base,
        status: unavailable || base.status === 'unavailable' ? 'unavailable' : attention ? 'attention' : base.status,
        timeAttestationGovernance: journal
      };
    } catch (error) {
      return {
        ...
        timeAttestationGovernance: {
          status: 'unavailable', enabled: governance.enabled,
...
function publicDecision(decision) {
  if (!decision) return { cryptographicallyValid: true, operationallyAcceptable: true, status: 'acceptable', reasons: [] };
  return {
