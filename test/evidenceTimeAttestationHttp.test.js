...
function fixture() {
  ...
  return { handler, store, authority, otherAuthority, challengeCalls: () => challengeCalls };
}
...
test('callback rejects invalid asymmetric signatures with a signature challenge', async (t) => {
  const { handler, otherAuthority, challengeCalls } = fixture();
  const { server, port } = await listen(handler);
  t.after(() => closeServer(server));
  ...
  assert.equal(challengeCalls(), 0, 'invalid signatures must be rejected before archive lookup');
});
...
test('store rejects invalid callbacks before resolving tenant or archive state', () => {
  const { store, otherAuthority, challengeCalls } = fixture();
  assert.throws(
