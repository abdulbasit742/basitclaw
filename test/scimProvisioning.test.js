...
function fixture() {
  ...
  const accessController = createScimAccessController({ credentials: [{
    ...
    scopes: ['scim:read', 'scim:write'], status: 'active'
  }] });
...
async function request(server, path, { method = 'GET', body, headers = {} } = {}) {
  ...
  const response = await fetch(`http://127.0.0.1:${server.address().port}${path}`, {
    method,
    headers: { authorization: `Bearer scim-1.${secret}`, ...(body ? { 'content-type': 'application/scim+json' } : {}), ...headers },
...
test('SCIM create, read, patch, and deprovision use ETags and governance evidence', async (t) => {
  ...
  const patched = await request(server, `/scim/v2/Users/${id}`, {
    method: 'PATCH', headers: { 'if-match': 'W/"1"' }, body: {
...
test('SCIM rejects missing credentials, stale ETags, and unsupported filters', async (t) => {
  ...
  const created = await request(server, '/scim/v2/Users', { method: 'POST', body: user });
  const conflict = await request(server, `/scim/v2/Users/${created.payload.id}`, {
    method: 'PATCH', headers: { 'if-match': 'W/"0"' }, body: {
      schemas: ['urn:ietf:params:scim:api:messages:2.0:PatchOp'],
      Operations: [{ op: 'replace', path: WORKFORCE_EXTENSION, value: { reason: 'stale update', role: 'audit_manager' } }]
    }
  });
