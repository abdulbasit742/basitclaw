...
const attestationA = Object.freeze({ attestationId: `NTA-${'a'.repeat(32)}`, archiveId, providerId: 'tsa-a', keyId: 'key-a', timestamp: '2026-07-30T00:30:00.000Z' });
const attestationB = Object.freeze({ attestationId: `NTA-${'b'.repeat(32)}`, archiveId, providerId: 'tsa-b', keyId: 'key-b', timestamp: '2026-07-30T01:30:00.000Z' });
...
function storeFixture(options = {}) {
  ...
  return {
    ...
    store: createEvidenceTimeAttestationGovernanceStore({
      mode: 'shared-file', requiredForDisposition: options.requiredForDisposition ?? false,
      directory, encryptionKeys: { governance: encryptionKey }, encryptionPrimaryKeyId: 'governance',
      signingSecrets: { journal: signingKey }, signingPrimaryKeyId: 'journal', maxEvents: 1000,
      now: options.now ?? (() => new Date('2026-07-30T02:00:00.000Z'))
    })
  };
}
...
function providerRevocation(overrides = {}) {
  return {
    eventType: 'provider_revoked', providerId: 'tsa-a', effectiveAt: '2026-07-30T01:00:00.000Z',
    retroactive: false, reasonCode: 'provider_termination',
    reason: 'Provider contract and trust approval were withdrawn.',
    confirmation: 'REVOKE NOTARY PROVIDER tsa-a', ...overrides
  };
}
...
test('prospective provider revocation preserves earlier proof and excludes later attestations', () => {
  ...
  const later = { ...attestationA, attestationId: `NTA-${'e'.repeat(32)}`, timestamp: '2026-07-30T01:30:00.000Z' };
  const decisions = store.evaluate(tenantId, [attestationA, later]).decisions;
  assert.equal(decisions.get(attestationA.attestationId).operationallyAcceptable, true);
  assert.equal(decisions.get(later.attestationId).operationallyAcceptable, false);
  assert.equal(decisions.get(later.attestationId).cryptographicallyValid, true);
});

test('retroactive key compromise excludes historical attestations without deleting them', () => {
  const { store } = storeFixture();
  store.record(tenantId, {
    eventType: 'key_revoked', providerId: 'tsa-a', keyId: 'key-a', effectiveAt: '2026-07-30T01:00:00.000Z',
    retroactive: true, reasonCode: 'key_compromise', reason: 'The authority signing key was reported compromised.',
...
test('supersession marks only the original attestation as operationally superseded', () => {
  const { store } = storeFixture();
  store.record(tenantId, {
    eventType: 'attestation_superseded', archiveId, attestationId: attestationA.attestationId,
    replacementAttestationId: attestationB.attestationId, effectiveAt: '2026-07-30T01:45:00.000Z',
    retroactive: false, reasonCode: 'superseded', reason: 'A corrected authority token replaces the original token.',
...
test('identical events are idempotent, strictly shaped and encrypted without plaintext identifiers', () => {
  ...
  assert.throws(() => store.record(tenantId, { ...providerRevocation(), hiddenPayload: 'no' }, { actor: 'manager.one' }), EvidenceValidationError);
...
function fakeTimeRegistry() {
  const item = { evidenceId, status: 'active', retentionUntil: '2035-07-30T00:00:00.000Z', versions: [{ version: 1, sha256: 'f'.repeat(64) }] };
  const receipt = { archiveId };
  return {
    recordTimeAttestation(input) { return { accepted: true, duplicate: false, attestation: input }; },
    verifyEvidenceTimeAttestations() { return { valid: true, archiveId, attestationCount: 2, distinctProviders: 2, minimumProviders: 2, quorumSatisfied: true, providerIds: ['tsa-a', 'tsa-b'] }; },
    evidenceTimeAttestations() { return [attestationA, attestationB]; },
    dispose() { return { status: 'disposed' }; },
    get() { return item; }, list() { return [item]; },
    evidencePreservationStore: { verifiedForVersion() { return receipt; } },
    evidenceTimeAttestationStore: { minimumProviders: 2, list() { return [attestationA, attestationB]; } },
    evidenceTimeAttestationStatus() { return { status: 'ready', dispositionReady: true }; },
    evidencePreservationReceipts() { return [receipt]; },
    verify() { return { valid: true }; }, health() { return { status: 'ready', required: true }; },
    tenantStatus() { return { status: 'ready' }; }, evidenceTimeAttestationEnabled: true
  };
}
...
test('retroactive provider compromise recalculates quorum and blocks disposition', () => {
  ...
  const registry = createEvidenceTimeAttestationGovernanceRegistry({ registry: fakeTimeRegistry(), governance: store });
  ...
  registry.recordTimeAttestationGovernanceEvent(tenantId, providerRevocation({
    retroactive: true, reasonCode: 'authority_compromise', reason: 'The authority trust boundary was reported compromised.'
  }), { actor: 'manager.one' });
  ...
  assert.throws(() => registry.dispose(tenantId, evidenceId, {}, { actor: 'admin.one' }), EvidenceTimeAttestationGovernanceRequiredError);
});

test('registry enforces exact confirmations and validates attestation targets', () => {
  const registry = createEvidenceTimeAttestationGovernanceRegistry({ registry: fakeTimeRegistry(), governance: storeFixture().store });
  assert.throws(() => registry.recordTimeAttestationGovernanceEvent(tenantId, { ...providerRevocation(), confirmation: 'REVOKE PROVIDER tsa-a' }, { actor: 'manager.one' }), EvidenceValidationError);
  assert.throws(() => registry.recordTimeAttestationGovernanceEvent(tenantId, {
    eventType: 'attestation_revoked', archiveId, attestationId: `NTA-${'9'.repeat(32)}`,
...
function post(port, body) {
  ...
  return new Promise((resolve, reject) => {
    const req = httpRequest({ host: '127.0.0.1', port, method: 'POST', path: '/api/workforce-audit/evidence-notary/governance/events', headers: { 'content-type': 'application/json', 'content-length': payload.length, connection: 'close' } }, (res) => {
      const chunks = [];
...
function baseAppForPermissions(permissions) {
  ...
  app.authenticationGateway = {
    mode: 'api-key', async authenticate() { return { subject: 'user.one', tenantId, keyId: 'key-one', permissions }; },
    authorise(principal, permission) { if (!principal.permissions.includes(permission)) throw new AuthorizationError(); return principal; }
  };
  app.apiSecurity = {};
  return app;
}
function fakeGovernanceHttpRegistry() {
  return {
    recordTimeAttestationGovernanceEvent() { return { recorded: true, duplicate: false, event: { eventId: 'NGE-test', eventType: 'provider_revoked', reasonCode: 'provider_termination' } }; },
...
test('runtime maps governance writes to the existing manager-only preservation permission', async (t) => {
  const app = createEvidenceTimeAttestationGovernanceAwareApp({ evidenceRegistry: fakeGovernanceHttpRegistry(), baseApp: baseAppForPermissions(['evidence:preserve', 'governance:read']), rateLimiter: null });
  const port = await listen(app);
  t.after(() => { app.closeIdleConnections?.(); app.closeAllConnections?.(); return new Promise((resolve) => app.close(resolve)); });
  assert.equal((await post(port, providerRevocation())).status, 201);
});

test('runtime denies governance writes to principals without preservation permission', async (t) => {
  const app = createEvidenceTimeAttestationGovernanceAwareApp({ evidenceRegistry: fakeGovernanceHttpRegistry(), baseApp: baseAppForPermissions(['governance:read']), rateLimiter: null });
  const port = await listen(app);
  t.after(() => { app.closeIdleConnections?.(); app.closeAllConnections?.(); return new Promise((resolve) => app.close(resolve)); });
  assert.equal((await post(port, providerRevocation())).status, 403);
});
